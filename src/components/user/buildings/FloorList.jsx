...
const FloorList = ({ data, buildingId }) => {
  return (
    <div className="p-4 rounded-xl shadow-md grid grid-cols-1 lg:grid-cols-6 gap-5 xl:gap-10">
      ...
      />
      <div>
        <h5 className="text-base md:text-xl font-bold text-black">{data?.floorName}</h5>
        <div className="flex items-center gap-1">
          <Image src="/svgs/user/basement.svg" width={18} height={18} alt="image" />
      ...
      />
      <List icon={'/svgs/user/toilet.svg'} title="Free Restrooms" value={data?.freeRestrooms} />
      <List icon={'/svgs/user/sensors.svg'} title="Active Sensors" value={data?.activeSensors} />
      <div className="mt-4 md:mt-6 flex items-center justify-center">
        <Link
          href={`/floor/floor-detail/${buildingId}--${data.id}`}
